...
from roi_data.loader import RoiDataLoader, MinibatchSampler, BatchSampler, collate_minibatch, RoiDataLoader_tuple, MultiRoiDataLoader, RoiDataLoader_BIN
# from modeling.model_builder import Generalized_RCNN
from modeling import model_builder_oicr_cbl
...
def save_ckpt(output_dir, args, step, train_size, model, optimizer):
    ...
    if cfg.MODEL.METHOD == "oicr_momentum":
        save_dict['momentum'] = {'cls_feats': model.cls_feats,
                                 'cls_feats_scores': model.cls_feats_scores,
                                 'cls_feats_iters': model.cls_feats_iters
                                }
...
def main():
    ...
    if args.dataset == "coco2014":
        ...
        cfg.MODEL.NUM_CLASSES = 20
    elif args.dataset == 'voc2012train':
    ...
    batchSampler = BatchSampler(
        sampler=MinibatchSampler(ratio_list, ratio_index, sample_type=cfg.SAMPLE_TYPE, random=cfg.RANDOM),
        batch_size=args.batch_size,
        drop_last=True
    )
    ...
    ### Model ###
    print(cfg.MODEL.METHOD)
    pcl = model_builder_oicr_cbl.Generalized_RCNN()
        ...
        for step in range(args.start_step, cfg.SOLVER.MAX_ITER):
            ...
            for inner_iter in range(args.iter_size):
                ...
                for key in input_data:
                    if key not in ['roidb', 'indexes', 'im_scales', 'im_scales_bin']:
                        # roidb is a list of ndarrays with inconsistent length
                        input_data[key] = list(map(Variable, input_data[key]))
                keys_needed = ["data", "rois", "labels"]
                keys_needed.extend(["indexes", "im_scales"])

                input_data = {k:v for k,v in input_data.items() if k in keys_needed}
                input_data["step"] = [step]
                index_now = int(input_data["indexes"][0].squeeze(dim=0))
                input_data["roi"] = [roidb[index_now]]
            ...
            optimizer.step()
            if cfg.OICR.CBL.MeanTeacher_FC or cfg.OICR.CBL.MeanTeacher_ALL or cfg.OICR.CBL.MeanTeacher_ROIFC:
                pcl.module.update_ema_variables(cfg.OICR.CBL.MeanTeacher_ALPHA, step)
            training_stats.IterToc()
            training_stats.LogIterStats(step, lr)

            if ((step+1) % CHECKPOINT_PERIOD == 0) or ((step+1) in cfg.TRAIN.SAVE_ITERS):
            # if (step) % 1 == 0:

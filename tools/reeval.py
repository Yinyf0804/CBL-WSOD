    ...
    for i, entry in enumerate(roidb):
        ...
        if test_corloc:
            ...
            _, _, cls_boxes_i = box_results_with_nms_and_limit(boxes['scores'],
                                                         boxes['boxes'])
            final_boxes_dict[entry['image']] = cls_boxes_i
        extend_results(i, final_boxes, cls_boxes_i)

    res_file = os.path.join(args.output_dir, 'detect_final.pkl')
    save_object(final_boxes_dict, res_file)

...
class JsonDataset(object):
    ...
    def _add_proposals_from_file(
        ...
        if 'train' in proposal_file:
            box_sel_num = cfg.TRAIN.PROPOSAL_NUMS
        else:
            box_sel_num = -1
        for i, entry in enumerate(roidb):
            ...
            if box_sel_num != -1:
                if cfg.TRAIN.PROPOSAL_RANDOM:
                    box_sel_num = min(box_sel_num, len(boxes))
                    rand_ind = np.random.randint(0, len(boxes), size=box_sel_num)
                    boxes = boxes[rand_ind]
                else:
                    boxes = boxes[:box_sel_num]
            # Sanity check that these boxes are for the correct image id
            # print(entry['id'], proposals[id_field][i])
            # print(type(entry['id']), type(proposals[id_field][i]))
            # d = Differ()
            # diff = d.compare(entry['id'].splitlines(), proposals[id_field][i].splitlines())
            # print('\n'.join(list(diff)))
    ...
    def _add_gt_annotations_withbox(self, entry):
        """Add ground truth annotation metadata to an roidb entry."""
        ann_ids = self.COCO.getAnnIds(imgIds=entry['id'], iscrowd=None)
        objs = self.COCO.loadAnns(ann_ids)
        # Sanitize bboxes -- some are invalid
        ...
        for cls in gt_classes:
            entry['gt_classes'][0, cls] = 1
        entry['box_cls'] = gt_classes
        entry['objects'] = valid_objs

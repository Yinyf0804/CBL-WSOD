...
def im_detect_all(model, im, box_proposals=None, timers=None, entry=None):
    ...
    if cfg.TEST.BBOX_AUG.ENABLED:
        ...
        scores, boxes, im_scale, blob_conv = im_detect_bbox(
            model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals, entry)
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None, entry=None):
    ...
    if cfg.MODEL.METHOD == "oicr_momentum":
        inputs['roi'] = [entry]
        inputs['im_scales'] = [im_scale]

    return_dict = model(**inputs)

    # cls prob (activations after softmax)
    if 'oicr' in cfg.MODEL.METHOD and ('refine_score' in return_dict):
        if cfg.OICR.Test_RFFINE_NUM > len(return_dict['refine_score']):
            scores = return_dict['refine_score'][0].data.cpu().numpy().squeeze()
            # refine_times = cfg.REFINE_TIMES + 1 if cfg.OICR.Test_Need_Regcls else cfg.REFINE_TIMES
            refine_times = len(return_dict['refine_score'])
            for i in range(1, refine_times):
                scores += return_dict['refine_score'][i].data.cpu().numpy().squeeze()
            scores /= refine_times
        else:
            scores = return_dict['refine_score'][cfg.OICR.Test_RFFINE_NUM].data.cpu().numpy().squeeze()
        # print(boxes)
        if return_dict['rois'].shape[1] == 5:
            boxes = return_dict['rois'][:, 1:] / im_scale[0]
        else:
            boxes = return_dict['rois'] / im_scale[0]
        # print(boxes)
        # input()
        ...
        bg_score = np.zeros((scores.shape[0], 1))
        scores = np.hstack((bg_score, scores))
...
def box_results_for_corloc(scores, boxes):  # NOTE: support single-batch
    ...
    for j in range(1, num_classes):
        ...
        if cfg.OICR.Need_Reg:
            boxes_j = boxes[max_ind, j * 4:(j + 1) * 4]
        else:
            boxes_j = boxes[max_ind, :]
        cls_boxes[j] = np.hstack((boxes_j.reshape(1, -1),
...
def box_results_with_nms_and_limit(scores, boxes, Need_Reg=False):  # NOTE: support single-batch
    ...
    for j in range(1, num_classes):
        ...
        if cfg.OICR.Need_Reg or Need_Reg:

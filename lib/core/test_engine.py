...
def test_net(
    ...
    for i, entry in enumerate(roidb):
        ...
        cls_boxes_i = im_detect_all(model, im, box_proposals, timers, entry)
...
def initialize_model_from_cfg(args, gpu_id=0):
    ...
    """
    model = model_builder_oicr_cbl.Generalized_RCNN()
    model.eval()
...
def get_roidb_and_dataset(dataset_name, proposal_file, ind_range, gt=False):

...
def ratios_dis(ratios1, ratios2):
    dis = np.abs(ratios1 - ratios2.reshape(-1, 1))
    return dis
...
def clip_boxes_2(boxes, im_shape):
...
def encode(reference_boxes, proposals):
        """
...
def decode(rel_codes, boxes, resize=8):
    """
    ...
    bbox_xform_clip=math.log(1000. / resize)
...
def box_iou_torch(box1, box2):
    ...
    area_1 = (box1[:, 2] - box1[:, 0] + TO_REMOVE) * (box1[:, 3] - box1[:, 1] + TO_REMOVE)
    area_2 = (box2[:, 2] - box2[:, 0] + TO_REMOVE) * (box2[:, 3] - box2[:, 1] + TO_REMOVE)
